...
public class Main {
    ...
    private static void commandLineArgs(String arguments[]) {
        ...
        if (length > 0) {
            // catch arrayOutOfBoundsException for bad argument list
            try {
                for (i = 0; i < length; i++) {
    ...
    private static void setInputSource(String fileName) {
        ...
        try {
            ...
            // continue with System.in  unchanged
            return;
        }
    ...
    private static void setOutputSink(String fileName) {
        ...
        try {
            ...
            // continue with System.out  unchanged
            return;
        }
    ...
    private static void setErrorSink(String fileName)
    {
        ...
        try {
            ...
            // continue with System.err  unchanged
            return;
        }
    ...
    public static void main(String argv[]) {
        ...
        if (dumpAST2) {
            try {
                if (compilerDumpFileName.length() > 0) {
                    if (!dumpAST1) {
                        // dump stream wasn't opened above
                        dumpFile = new File(compilerDumpFileName) ;
                        dumpStream = new PrintStream(new FileOutputStream(dumpFile));
                    }
        ...
        try {
            ...
            codegen.Initialize();
            codegen.Generate(programAST, lines);
            if(codegen.Finalize())
